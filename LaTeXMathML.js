...
//{input:"\\mathop", tag:"munder", output:"\u00AF", ttype:UNARY, acc:true},
//{input:"underline",	tag:"munder", output:"\u0332", ttype:UNARY, acc:true},
{input:"\\label", tag:"mstyle", output:"\\mathrm", ttype:DEFINITION},
{input:"\\limits_",   tag:"munder",  output:"",  ttype:INFIX},
...
{input:"\\mathop",tag:"mstyle",atname:"mathvariant", atval:"fraktur",ttype:UNARY, codes:AMfrk},
{input:"\\bm", tag:"mstype", atname:"mathvariant", atval:"bold", ttype:UNARY},
{input:"\\rm", tag:"mstype", atname:"mathvariant", atval:"roman", ttype:UNARY}
];
...
var transEquation = (function(nEquation, delim) {  
  return function(str) {
    var patt = "\\end\{equation\}";
    var spaces = "\\textrm\{      \}";
    ...
    while (index >= 0) {
      ...
      if (index == 0 || str[index-1] != '\\') {
        nEquation++;
        label = spaces + "\\mathrm\{(" + nEquation + ")\}" + label;
        str = str.slice(0,index) + label + str.slice(index+patt.length);
      }
      index = str.indexOf(patt, index+label.length);
    }
    str = str.replace(/([^\\])\\begin\{equation\}/ig, "$1" + delim);
    str = str.replace(/^\\begin\{equation\}/ig, delim);    
    return str;
  }
})(0, " \$");
...
function AMprocessNodeR(n, linebreaks) {
  ...
  if (n.childNodes.length == 0) {
   if ((n.nodeType!=8 || linebreaks) &&
    ...
    true//n.parentNode.nodeName!="pre" && n.parentNode.nodeName!="PRE"
    ) {
    str = n.nodeValue;
    if (!(str == null)) {
      str = str.replace(/\r\n\r\n/g,"\n\n");
